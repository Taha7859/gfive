...
export default function EmotesPage() {
  ...
  // Fetch Emotes from Sanity
  useEffect(() => {
    const fetchEmotes = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="streamgraphics" && lower(category)=="emotes" && lower(subType) == lower($sub)]
  ...
  return (
    <>
      <Head>
        <title>{activeSub === "static" ? "Static Emotes" : "Animated Emotes"} | Moon Designs</title>
        <meta
          name="description"
          content={`Explore high-quality ${activeSub} emotes — Premium, Standard, and Basic designs by Moon Designs.`}
        />
        <meta
          name="keywords"
          content={`${activeSub} emotes, stream graphics, Moon Designs`}
        />
      </Head>
      ...
      <div className="min-h-screen bg-white select-none text-gray-900">
        ...
        {/* Toggle Buttons */}
        <div className="flex justify-center gap-3 p-6 pt-28 mb-10">
          <HoverButton
            onClick={() => setActiveSub("static")}
            className={`border text-sm font-medium transition ${
              activeSub === "static"
                ? "bg-black text-white border-black"
                : "bg-white text-gray-700 hover:text-white border-gray-300 hover:bg-gray-100"
            }`}
          >
            Static Emotes
          </HoverButton>
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">No {activeSub} emotes found</p>
        ) : (
          chunkedItems.map((group, groupIndex) => (
            <div key={groupIndex} className="mb-10">
              {group[0]?.label && (
                <h2 className="text-xl font-semibold text-center mb-4">{group[0].label}</h2>
              )}
