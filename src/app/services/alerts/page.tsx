...
export default function AlertsPage() {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="streamgraphics" && lower(category)=="alerts" && lower(subType)==lower($sub)] | order(_createdAt asc){
          _id,
          title,
  ...
  return (
    <>
      <Head>
        <title>{activeSub === "static" ? "Static Alerts | Moon Designs" : "Animated Alerts | Moon Designs"}</title>
      </Head>
      ...
      <div className="min-h-screen bg-white text-gray-900 select-none">
        ...
        <div className="p-6 pt-28">
          ...
          {loading ? (
            ...
            <p className="text-center text-gray-500">No {activeSub} alerts found</p>
          ) : (
