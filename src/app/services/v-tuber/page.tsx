...
export default function VtuberPage() {
  const [items, setItems] = useState<VtuberItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedImage, setSelectedImage] = useState<VtuberItem | null>(null);
  ...
  useEffect(() => {
    const fetchVtuber = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="streamgraphics" && lower(category)=="v tuber" && lower(subType)=="animated"] | order(_createdAt asc){
          _id,
          title,
  ...
  return (
    <>
      <Head>
        <title>Animated Vtuber Models | ShopFusion</title>
        <meta
          name="description"
          content="High-quality animated Vtuber models — Premium, Standard, and Basic models by ShopFusion."
        />
        <meta
          name="keywords"
          content="vtuber models, animated vtuber, ShopFusion, streaming assets"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white text-gray-900 select-none">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No Vtuber models found</p>
          ) : (
