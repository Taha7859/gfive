...
export default function KawaiiCharactersPage() {
  ...
  // Fetch Kawaii characters from Sanity
  useEffect(() => {
    const fetchKawaii = async () => {
      try {
        setLoading(true);
        const query = `
          *[_type=="characterDesign" && lower(category)=="kawaiicharacters"]
  ...
  return { ...item, label };
        ...
        console.error("❌ Error fetching Kawaii characters:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Kawaii Characters | Moon Designs</title>
        <meta
          name="description"
          content="Explore adorable Kawaii character designs — Premium, Standard, and Basic by Moon Designs. Dynamic pricing from Sanity CMS."
        />
        <meta
          name="keywords"
          content="Kawaii Characters, Moon Designs, Premium Kawaii, Standard Kawaii, Basic Kawaii, character design"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white select-none text-gray-900">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No Kawaii characters found</p>
          ) : (
