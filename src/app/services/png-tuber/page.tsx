...
export default function PngTuberPage() {
  const [items, setItems] = useState<PngTuberItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedImage, setSelectedImage] = useState<PngTuberItem | null>(null);
  ...
  // ✅ Fetch PNGTuber from Sanity (Static)
  useEffect(() => {
    const fetchPNGTuber = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="streamgraphics" && lower(category)=="png tuber" && lower(subType)=="static"] | order(_createdAt asc){
          _id,
          title,
        ...
        const data: PngTuberItem[] = await client.fetch(query);

        // ✅ Add dynamic label: Premium / Standard / Basic based on index
        ...
        console.error("❌ Error fetching PNGTuber:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // ✅ Chunk items for 3-per-row display
  const chunkedItems = useMemo(() => {
    const chunks: PngTuberItem[][] = [];
  ...
  return (
    <>
      <Head>
        <title>Static PNGTuber Models | ShopFusion</title>
        <meta
          name="description"
          content="Explore high-quality static PNGTuber models — Premium, Standard, and Basic designs by ShopFusion."
        />
        <meta
          name="keywords"
          content="png tuber, vtuber png, static pngtuber, ShopFusion, stream graphics"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white text-gray-900 select-none">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No PNGTuber models found</p>
          ) : (
                  ...
                  {group.map((item) => (
                    ...
                    >
                      <Image
                        ...
                        width={600}
                        height={400}
                        className="w-full h-48 sm:h-56 lg:h-64 xl:h-80 object-contain transition-transform duration-300 group-hover:scale-105 pointer-events-none"
                        draggable={false}
                        unoptimized
                        priority
                      />
