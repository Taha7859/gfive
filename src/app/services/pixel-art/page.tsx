...
export default function PixelArtCharactersPage() {
  ...
  // Fetch Pixel Art characters from Sanity
  useEffect(() => {
    const fetchPixelArt = async () => {
      try {
        setLoading(true);
        const query = `
          *[_type=="characterDesign" && lower(category)=="pixelartcharacters"]
  ...
  return { ...item, label };
        ...
        console.error("❌ Error fetching Pixel Art characters:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Pixel Art Characters | Moon Designs</title>
        <meta
          name="description"
          content="Explore high-quality Pixel Art character designs — Premium, Standard, and Basic by Moon Designs. Dynamic pricing from Sanity CMS."
        />
        <meta
          name="keywords"
          content="Pixel Art Characters, Moon Designs, Premium Pixel Art, Standard Pixel Art, Basic Pixel Art, character design"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white select-none text-gray-900">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No Pixel Art characters found</p>
          ) : (
