...
export default function CharacterArtPage() {
  ...
  // Fetch Character Art
  useEffect(() => {
    const fetchCharacterArt = async () => {
      try {
        setLoading(true);
        const query = `
          *[_type=="characterDesign" && lower(category)=="characterart"]
  ...
  return { ...item, label };
        ...
        console.error("❌ Error fetching Character Art:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Character Art | Moon Designs</title>
        <meta
          name="description"
          content="Explore high-quality Character Art — Premium, Standard, and Basic designs by Moon Designs. Dynamic pricing from Sanity CMS."
        />
        <meta
          name="keywords"
          content="Character Art, Moon Designs, Premium Art, Standard Art, Basic Art, character design"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white select-none text-gray-900">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No Character Art found</p>
          ) : (
