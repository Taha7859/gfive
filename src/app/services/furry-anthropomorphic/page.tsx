...
export default function FurryAnthropomorphicPage() {
  const [items, setItems] = useState<CharacterItem[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [selectedImage, setSelectedImage] = useState<CharacterItem | null>(null);

  // Disable right-click and double-click protection
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const query = `
          *[_type=="characterDesign" && lower(category)=="furryanthropomorphiccharacters"]
        ...
        // Auto-assign Premium / Standard / Basic based on index
  ...
  // Efficient chunking (Memoized to avoid re-renders)
  ...
  return (
    <>
      <Head>
        <title>Furry / Anthropomorphic Characters | shpfusion.com</title>
        <meta
          name="description"
          content="Buy high-quality Furry / Anthropomorphic character designs. Premium, Standard, and Basic tiers loaded directly from Sanity CMS."
        />
        <meta
          name="keywords"
          content="Furry Characters, Anthropomorphic Art, Character Design, Premium Characters, Sanity CMS"
        />
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">
              No Furry / Anthropomorphic Characters found.
            </p>
          ) : (
