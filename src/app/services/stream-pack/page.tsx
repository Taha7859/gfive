...
export default function StreamPackAnimatedPage() {
  const [items, setItems] = useState<StreamPackItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedImage, setSelectedImage] = useState<StreamPackItem | null>(null);
  ...
  // Fetch Stream Pack Animated items
  useEffect(() => {
    const fetchItems = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="streamgraphics" && lower(category)=="stream pack" && lower(subType)=="animated"] | order(_createdAt asc){
          _id,
          title,
          "imageUrl": image.asset->url,
          price
        }`;
        const data: StreamPackItem[] = await client.fetch(query);
  ...
  return { ...item, label };
        ...
        console.error("❌ Error fetching Stream Pack items:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Animated Stream Packs | ShopFusion</title>
        <meta
          name="description"
          content="Explore high-quality animated stream packs — Premium, Standard, and Basic designs by ShopFusion."
        />
        <meta
          name="keywords"
          content="animated stream packs, stream graphics, ShopFusion, Twitch overlays, animated overlays"
        />
      </Head>
      ...
      <div className="min-h-screen bg-white text-gray-900 select-none">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No animated stream packs found</p>
          ) : (
