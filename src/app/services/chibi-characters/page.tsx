...
type CharacterItem = {
  ...
  category: string;
  price: number;
  label?: string; // Optional label for UI grouping
};

export default function ChibiCharactersPage() {
  ...
  // ✅ Fetch Chibi Characters dynamically from Sanity
  useEffect(() => {
    const fetchChibi = async () => {
      setLoading(true);
      try {
        const query = `*[_type=="characterDesign" && lower(category) == "chibicharacters"] 
        ...
        // ✅ Add optional price label based on index
  ...
  return { ...item, label };
        ...
        console.error("Error fetching Chibi characters:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
  <title>Chibi Characters | shpfusion.com</title>
  <meta
    name="description"
    content="Discover adorable Chibi Characters — Premium, Standard, and Basic designs by shpfusion.com. Fully dynamic, fast loading, and SEO optimized."
  />
  <meta
    name="keywords"
    content="Chibi Characters, shpfusion.com, Premium Chibi, Standard Chibi, Basic Chibi, character designs"
  />
      ...
      <div className="min-h-screen bg-white select-none text-gray-900">
        ...
        <div className="p-6 pt-28">
          {loading ? (
            ...
            <p className="text-center text-gray-500">No Chibi characters found</p>
          ) : (
                  ...
                  {group.map((item) => (
                    ...
                    >
                      <Image
                        ...
                        loading="lazy" // ✅ Lazy load for faster page render
                        unoptimized
                        draggable={false}
                      />
